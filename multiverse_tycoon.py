...
class MultiVerseTycoon:
    ...
    def check_level_up(self):
        ...
        if self.player["experience"] >= xp_threshold:
            ...
            # Check for victory condition at level 10
            if self.player["player_level"] >= 10:
                # Check if the player meets victory requirements
    ...
    def check_victory_condition(self):
        """Check if player meets all requirements for the victory ending."""
        ...
        # Victory is only triggered if player has established substantial presence across most universes
        # and accumulated significant quantum credits
        if total_quantum >= 5000 and total_businesses_all >= 20 and universes_with_businesses >= len(self.universes) - 1:
            self.trigger_victory_ending()
            return True
        return False
    
    def trigger_victory_ending(self):
        """Trigger the ultimate victory ending when all conditions are met."""
        self.clear_screen()
        ...
        self.slow_print("FROM: Dr. Eleanor Quantum, Founder & CEO")
        self.slow_print(f"TO: {self.player['name']}, Multiverse Operations Director")
        self.slow_print("SUBJECT: The Nexus Event\n")
        
        self.slow_print("It is with profound gratitude and awe that I reach out to you today.")
        self.slow_print("What we've achieved together transcends all of our expectations.")
        
        self.slow_print("\nThe Quantum Stabilization Matrix is now operating at 100% efficiency")
        self.slow_print("across all monitored universes. The fractures that once threatened")
        self.slow_print("to collapse our multiverse have not only been sealed but have been")
        self.slow_print("transformed into something altogether new.")
        
        self.slow_print("\nOur scientists have detected what we're calling the 'Nexus Event' - ")
        self.slow_print("a fundamental restructuring of the multiversal fabric. Your business")
        self.slow_print("network, spanning across all identified realities, has created a")
        self.slow_print("self-reinforcing quantum lattice that has essentially 'upgraded'")
        self.slow_print("the multiverse itself.")
        
        self.slow_print("\nThe barriers between universes are now permanently stabilized, allowing")
        self.slow_print("for safe, controlled travel between realities. More importantly, we've")
        self.slow_print("detected the formation of new universes - realities that didn't exist")
        self.slow_print("before our intervention. The multiverse is growing, healing, evolving.")
        
        self.slow_print("\nThe Board has unanimously voted to officially rename the company")
        self.slow_print("to 'Nexus Corporation' and has created a new position specifically")
        self.slow_print("for you: Supreme Architect of the Multiverse.")
        
        self.slow_print("\nIn addition, we've discovered something extraordinary. The quantum flux")
        self.slow_print("generated by your business empire has begun affecting the fundamental")
        self.slow_print("nature of reality itself. Small changes at first - manipulating probability,")
        self.slow_print("accelerating technological advancement, even affecting the flow of time.")
        
        self.slow_print("\nIt appears that with sufficient quantum energy, we may be able to")
        self.slow_print("reshape reality according to our will. The implications are staggering.")
        self.slow_print("You've not only saved the multiverse - you've given us the keys to creation itself.")
        
        self.slow_print("\nA new era begins today. And you, my friend, are its architect.")
        
        self.slow_print("\nWith boundless gratitude and respect,")
        self.slow_print("Dr. Eleanor Quantum")
        
        self.slow_print("\n[END TRANSMISSION]")
        
        # Small epilogue
        self.slow_print("\n...\n")
        self.slow_print("As you lower the quantum tablet, you gaze out at the shimmering")
        self.slow_print("interdimensional portal before you - a stable, permanent gateway")
        self.slow_print("between universes of your own creation. Through it, you can see")
        self.slow_print("fragments of countless realities - some familiar, others entirely new.")
        
        self.slow_print("\nYour journey began as a simple entrepreneur, traversing dangerous")
        self.slow_print("parallel worlds to build a business empire. Now, you stand as the")
        self.slow_print("Supreme Architect of the Multiverse, with the power to shape reality itself.")
        
        self.slow_print("\nThe question is no longer what you will build next, but what kind")
        self.slow_print("of multiverse you wish to create.")
        
        self.slow_print("\nThe possibilities are infinite.")
        
        # Record victory in player state for future reference
        if not "victory_achieved" in self.player:
            self.player["victory_achieved"] = True
            self.player["victory_turn"] = self.player["turn"]
        
        input("\nPress Enter to continue your journey as the Supreme Architect of the Multiverse...")
    ...
    def game_over(self):
        ...
        total_businesses_all = sum(len(universe_data['businesses']) for universe_data in self.player["universes"].values())
        universes_with_businesses = sum(1 for universe_data in self.player["universes"].values() if len(universe_data['businesses']) > 0)
        total_wealth = sum(universe_data['cash'] for universe_data in self.player["universes"].values())
        max_danger = max(universe_data['danger'] for universe_data in self.player["universes"].values()) if self.player["universes"] else 0
        
        # Exceptional success (legendary ending)
        if total_quantum > 3000 and total_businesses_all > 15 and universes_with_businesses >= 5:
            self.slow_print("\n[CONFIDENTIAL: EYES ONLY]")
            self.slow_print("\nOperative, your remarkable success has exceeded our wildest expectations.")
            self.slow_print("The Quantum Nexus has stabilized beyond our projections, and we've detected")
            self.slow_print("something extraordinary - a new phenomenon we're calling 'Dimensional Resonance.'")
            
            self.slow_print("\nYour business network has created a self-sustaining energy loop across")
            self.slow_print("the multiverse, effectively making the barriers between universes permeable")
            self.slow_print("in a controlled manner. We've begun establishing authorized transit points")
            self.slow_print("for secure travel between dimensions.")
            
            self.slow_print("\nThe Multiverse Council has unanimously voted to appoint you as the first")
            self.slow_print("Interdimensional Ambassador with full diplomatic authority across all realities.")
            self.slow_print("Your knowledge and experience are invaluable as we begin a new era of")
            self.slow_print("peaceful multiversal cooperation and commerce.")
            
            self.slow_print("\nCongratulations, Ambassador. History will remember you as the architect")
            self.slow_print("of the Quantum Age. The multiverse is safe in your hands.")
            
        # High success (good ending)
        elif total_quantum > 1000 and total_businesses_all > 10 and universes_with_businesses >= 3:
            self.slow_print("\n[MISSION UPDATE: CLASSIFIED]")
            self.slow_print("\nOperative, your contributions to our interdimensional network have been")
            self.slow_print("instrumental in our breakthrough. The quantum credits you've accumulated")
            self.slow_print("have powered our stabilization matrix beyond critical threshold.")
            
            self.slow_print("\nOur scientists have successfully created a stable Quantum Bridge")
            self.slow_print("that allows controlled travel between universes without risking collapse.")
            self.slow_print("The fractures in the multiverse fabric are healing, and we've established")
            self.slow_print("permanent monitoring stations in each reality.")
            
            self.slow_print("\nThe Board has approved your promotion to Director of Multiverse Operations.")
            self.slow_print("Your new assignment will be to oversee the expansion of our interdimensional")
            self.slow_print("business network and train the next generation of operatives.")
            
            self.slow_print("\nThe multiverse is secure thanks to your efforts, Director. Humanity's")
            self.slow_print("future across all realities looks brighter than ever.")
            
        # Moderate success (neutral ending)
        elif total_quantum > 500 or total_businesses_all > 5:
            self.slow_print("\nYour efforts have provided valuable data for our stabilization project.")
            self.slow_print("While not entirely successful, your business network has slowed the")
            self.slow_print("degradation of the multiverse fabric. Our scientists now have the time")
            self.slow_print("they need to develop more effective stabilization technology.")
            
            self.slow_print("\nWe've identified several promising candidates to continue your work,")
            self.slow_print("armed with the knowledge and strategies we've gleaned from your operations.")
            self.slow_print("The quantum credits you've gathered will power our research division")
            self.slow_print("as we work to perfect the Nexus Protocol.")
            
            self.slow_print("\nYour service to the Quantum Corporation and the multiverse will not")
            self.slow_print("be forgotten. Rest assured, we will succeed where you left off.")
            
        # Dangerous situation (bad ending with hope)
        elif max_danger > 75:
            self.slow_print("\n[EMERGENCY PROTOCOL INITIATED]")
            self.slow_print("\nThe high detection levels across multiple universes have triggered")
            self.slow_print("a cascading quantum resonance that threatens to collapse the entire")
            self.slow_print("multiverse. Your interdimensional signature has been traced by multiple")
            self.slow_print("authorities, forcing us to initiate emergency extraction procedures.")
            
            self.slow_print("\nOur last resort technology - the Quantum Reset Engine - has been activated.")
            self.slow_print("This device will essentially 'reboot' the affected universes, erasing all")
            self.slow_print("evidence of interdimensional interference at the cost of significant energy.")
            
            self.slow_print("\nWe've salvaged what quantum credits we could from your operations to")
            self.slow_print("power this last-ditch effort. If successful, we'll have one more chance")
            self.slow_print("to establish a stable network with a new operative using lessons from your mission.")
            
            self.slow_print("\nStand by for dimensional reset in 3... 2... 1...")
            
        # Failure (true bad ending)
        else:
            self.slow_print("\n[CRITICAL ALERT: NEXUS DESTABILIZING]")
            self.slow_print("\nYour premature mission failure has triggered a catastrophic chain reaction")
            self.slow_print("in the multiverse fabric. Without sufficient quantum credits to stabilize")
            self.slow_print("the interdimensional rifts, we're detecting multiple universe collapse events.")
            
            self.slow_print("\nThe Quantum Corporation is initiating emergency protocols to preserve")
            self.slow_print("our home reality, but we can no longer save the other universes. Billions")
            self.slow_print("of lives across countless realities will be lost as the multiverse")
            self.slow_print("implodes into a singular timeline.")
            
            self.slow_print("\nOur sensors indicate that fragments of these dying realities may")
            self.slow_print("survive as pocket dimensions, but the grand multiverse experiment")
            self.slow_print("is effectively over. All we can do now is hope that someday,")
            self.slow_print("another reality discovers the Quantum Nexus and learns from our mistakes.")
